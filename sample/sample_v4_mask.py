...
def parse_args():
    ...
    parser.add_argument("--batch-size", type=int, default=512,
                        help="Per-target sampling batch size to avoid OOM on GPU.")
...
def _sample_batch_with_mask(
    ...
    conditions = torch.full((batch_size, cond_dim), float(norm_tg), device=device)

    z_base = torch.randn(batch_size, model.z_dim, device=device)
    ...
    cur = torch.full((batch_size, 1), bos_id, dtype=torch.long, device=device)
    ...
    for _ in range(max_len - 1):
        ...
        y = model._run_decoder(
            tgt=tgt,
            memory=memory.unsqueeze(1).expand(batch_size, T, -1),
            tgt_mask=tgt_mask,
...
def sample_smiles_with_mask(
    ...
    batch_cap = batch_cap or batch_size
    for t in targets:
        norm = tg_stats.normalize([t])[0]
        remaining = num_per_target
        while remaining > 0:
            cur_batch = min(batch_cap, remaining)
            decoded = _sample_batch_with_mask(
                model=model,
                tokenizer=tokenizer,
            ...
            sample_targets.extend([t] * len(decoded))
            remaining -= cur_batch
...
def main():
    ...
    samples, sample_targets = sample_smiles_with_mask(
        model=model,
        tokenizer=tokenizer,
        syntax_masker=syntax_masker,
        tg_stats=tg_stats,
