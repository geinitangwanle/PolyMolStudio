...
def parse_args():
    ...
    parser.add_argument("--batch-size", type=int, default=512,
                        help="Sampling batch size to control GPU memory.")
...
def _sample_batch(
    ...
    conditions = torch.zeros(batch_size, 1, device=device)
...
def sample_smiles(
    ...
    while remaining > 0:
        ...
        remaining -= cur_batch
        finished += cur_batch
        pct = (finished / num_samples * 100) if num_samples else 0.0
        print(f"[sampling] batch {batch_idx}: {finished}/{num_samples} ({pct:.1f}%)", flush=True)
        batch_idx += 1
    return samples
