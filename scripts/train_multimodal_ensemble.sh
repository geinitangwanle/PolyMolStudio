...
#SBATCH --output=log/%j_multi_ens.out

#SBATCH --error=log/%j_multi_ens.err
...
do
  echo "Training multimodal ensemble member with MODEL_SEED=${MODEL_SEED}"
  ...
  srun python ./train/train_predictor.py \
    ...
    --run_name multimodal_seed${MODEL_SEED} \
    --checkpoint_dir checkpoints/ensemble_multimodal \
    --log_dir logs/ensemble_multimodal \
    --device auto \
    --use_polybert \
    --polybert-dir ./polybert \
    --no-freeze_polybert \
    --polybert_train_last_n 2 \
    --polybert_lr 1e-5 \
    --seq_max_length 256 \
    --cross_attn_heads 4 \
    --cross_attn_dim 64
