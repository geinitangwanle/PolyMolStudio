#!/usr/bin/env python3
"""REINVENT training script for ConditionalVAESmiles (modelv4).

核心循环：sample → reward → logp(agent/prior) → REINVENT loss → backward。

默认 reward 是全 1，占位用；请根据需要替换 compute_reward 函数（例如加载 Tg 预测器、合法性/新颖性评分等）。
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="REINVENT training for modelv4")
    # data
    ...
    # optional Tg predictor (GeoGAT) for reward
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in pbar:
            ...
            conditions = batch["tg"].unsqueeze(1).to(device)  # [B,1]
            ...
            loss = rl_losses.reinvent_loss(logp_agent, logp_prior, reward, reduction="mean")
        ...
        ckpt_path = save_dir / f"reinvent_epoch{epoch:03d}.pt"
